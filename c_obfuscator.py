...
def main():
    ...
    if len(sys.argv) > file_arg_index:
        ...
        try:
            # エンコーディングを自動検出してファイルを読み込む
            source_code, detected_encoding = read_file_with_encoding(filename)
    ...
    if len(sys.argv) > file_arg_index:
        ...
        # 必要に応じて元のエンコーディングで保存することも可能
        output_encoding = 'utf-8'
        
        with open(output_filename, 'w', encoding=output_encoding) as f:
            f.write(transformed_code)
        
        with open(table_filename, 'w', encoding=output_encoding) as f:
            f.write(conversion_table)
        
        print(f"\n変換後のコードを '{output_filename}' に保存しました (エンコーディング: {output_encoding})")
